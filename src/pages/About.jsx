...
const S = {
  ...
  p: { fontSize: 14, color: '#22375A', lineHeight: 1.7, marginBottom: 16, maxWidth: 680 },
  ...
  note: { background: 'rgba(34,55,90,0.04)', borderLeft: '2px solid rgba(34,55,90,0.20)', borderRadius: '0 6px 6px 0', padding: '12px 16px', fontSize: 13, color: '#22375A', lineHeight: 1.6, marginBottom: 24, maxWidth: 720 },
  bio: { background: 'rgba(97,158,168,0.06)', border: '0.5px solid rgba(97,158,168,0.20)', borderRadius: 10, padding: '16px 20px', fontSize: 13, color: '#22375A', lineHeight: 1.6, marginTop: 32, marginBottom: 24, maxWidth: 720 },
};

export default function About() {
  const isMobile = useIsMobile();
  return (
    <div style={isMobile ? { ...S.page, padding: '32px 16px 64px' } : S.page}>
      ...
      <div style={S.note}>
        ...
        <p style={{ ...S.p, marginBottom: 8 }}><strong>External labour market dynamics are not modelled.</strong> The pool of candidates is treated as fixed and infinite. Peng & Garg (ICML 2025) show monoculture reinforces itself as more firms adopt the same tools, an omitted amplification.</p>
      ...
      <p style={S.p}>
        Six dynamic verifications (V1{'\u2013'}V6) confirmed the model's behavioural properties beyond the central paradox. V1 measured pairwise error correlation directly: C_excess rises from 0.045 to 0.064 under G0 over 20 quarters (+42%), validating the Wood et al. mechanism as a measured trajectory rather than an algebraic assumption. V2 confirmed bimodality is causally pure: 99.7% of quarters above the catastrophe threshold carry crisis_flag=True. V5 verified that the paradox holds without exception across <InlineMath math={String.raw`p_{\text{crisis}} \in \{0.04, 0.15\}`} />.
      </p>
      <p style={S.p}>
      ...
      <div style={isMobile ? { overflowX: 'auto', marginBottom: 24, WebkitOverflowScrolling: 'touch' } : undefined}>
      <table style={isMobile ? { ...S.table, minWidth: 560, marginBottom: 0 } : S.table}>
        <thead>
          <tr>
      ...
      <div style={isMobile ? { overflowX: 'auto', marginBottom: 24, WebkitOverflowScrolling: 'touch' } : undefined}>
      <table style={isMobile ? { ...S.table, minWidth: 460, marginBottom: 0 } : S.table}>
        <thead>
          <tr>
            <th style={S.th}>Parameter</th>
        ...
        <tbody>
          {[
            ...
            ['Diversity quota', '\u2013', '\u2013', '\u2013', '10%'],
            ['Velocity cost vs G0', 'N/A', 'reference', '\u22124.0%', '\u221212%'],
