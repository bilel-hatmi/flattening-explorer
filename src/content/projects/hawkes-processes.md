...
The project implemented estimation for a two-exponential Hawkes process and examined a supervised alternative for the one-exponential case, then compared **4 routes** on the same simulated streams: maximum likelihood, the Whittle spectral estimator, a uniform-binning approximation, and neural estimators (an MLP and an LSTM trained on simulated data). Fitting quality was also checked on real empirical event data.

The findings were clear-cut. **Maximum likelihood with random restarts was the most effective method on clean binned data**; when the data were degraded, **only the Whittle estimator remained usable**. Group project at CentraleSupélec, February to June 2023, supervised by Ioane Muni Toke; the report and the code are linked above.
